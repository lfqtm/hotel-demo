...
```java
# ES启动成功后,就是一个web项目(服务)
# 直接向ES服务发送RestFul风格的请求即可,携带Json格式的参数


# 格式: http://192.168.163.130:9200/请求路径
# 注意: 我们在安装Kibana时,已经设置了Kibana连接ES的地址因此前缀可以省去不写
#       直接从请求路径开始编写即可
...
# ==============================分词器
# ES是国外的产品,默认情况下携带了一个 standard 分词器(标准分词器)
# 请求方式 请求路径
GET /_analyze
{
  "analyzer": "standard",
...
# -------------------IK分词器(对中文进行分词)
# IK分词器是一个插件,需要在ES中安装此插件才可以使用
# 直接将ik分词器插件,存放到对应的数据卷中,并重启ES
...
@SpringBootTest
public class HotelDemo3RestApiBulk {
    ...
    public void addDoc() throws IOException {
        ...
        for (Hotel hotel : list) {
            // 将hotelDoc对象转成json
            String hotelDocJson = JSON.toJSONString(new HotelDoc(hotel));
            // 创建单个添加文档数据对象
...
![image-20220103101739554](assets/image-20220103101739554.png)
